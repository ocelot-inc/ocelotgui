# ocelotgui.spec file for version 2.1.0 supplied by Peter Gulutzan as part of ocelotgui package
...
# For example you might say: wget https://github.com/ocelot-inc/ocelotgui/releases/download/2.1.0/ocelotgui-2.1.0.tar.gz
# (For this step, we assume you know where you downloaded to. See later explanation in section "Re: Source".)
# You must copy it to $HOME/ocelotgui-2.1.0.tar.gz -- this is hard coded.
# If CmakeLists.txt was updated since the release, which is true for 2.1.0, Step #2 will not work -- use Step #2a instead.
# 2a. Alternative to step 2: make ocelotgui-2.1.0.tar.gz from a clone of the latest ocelotgui source.
# This is approximately the way that the ocelotgui developer makes new tar.gz files when making new releases.
# To produce $HOME/ocelotgui-2.1.0.tar.gz from the latest github source, say:
# cd /tmp
# rm -r -f ocelotgui
# git clone https://github.com/ocelot-inc/ocelotgui ocelotgui
# rm -r -f ocelotgui/.git
# tar -zcvf $HOME/ocelotgui-2.1.0.tar.gz ocelotgui
# rm -r -f ocelotgui
# 3. Remake ocelotgui-2.1.0.tar.gz so it will unpack to directory ocelotgui-2.1.0 rather than to ocelotgui.
# This step is necessary because ocelotgui.spec will look for files in ocelotgui-2.1.0.
# To change $HOME/ocelotgui-2.1.0.tar.gz to be ready for rpm, say:
# cd /tmp
# rm -r -f ocelotgui
# rm -r -f ocelotgui-2.1.0
# cp -p $HOME/ocelotgui-2.1.0.tar.gz ocelotgui-2.1.0.tar.gz
# tar -xf ocelotgui-2.1.0.tar.gz
# mv ocelotgui ocelotgui-2.1.0
# tar -zcvf $HOME/ocelotgui-2.1.0.tar.gz ocelotgui-2.1.0
...
# rpmlint ~/ocelotgui_rpm//rp/rpmbuild/RPMS/x86_64/ocelotgui-2.1.0-1.x86_64.rpm
...
#  and in any case it would unpack to ocelotgui not ocelotgui-2.1.0.
...
Url:            http://ocelot.ca
#Source0:        ocelotgui-2.1.0.tar.gz
Source:         https://github.com/ocelot-inc/%name/releases/download/2.1.0/%name-%{version}.tar.gz
